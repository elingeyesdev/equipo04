...
@section('content')
                                    ...
                                    <div class="flex items-center gap-3">
                                        @if(isset($role) && $role === 'authority' && $estado === 'activa')
                                            <form method="POST" action="{{ route('reports.desactivar', ['id' => $id], false) }}"
                                                  onsubmit="confirmForm(event, '¿Desactivar la inundación N°{{ $id }}? Pasará a estado Terminada.')" onclick="event.stopPropagation()">
                                                @csrf
                                ...
                                <div class="p-5 flex flex-col flex-grow">
                                    ...
                                    @else
                                        <div class="mb-4 w-full h-32 rounded-xl bg-slate-100 flex flex-col items-center justify-center border border-dashed border-slate-300">
                                            <svg class="w-8 h-8 opacity-50 mb-1 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                                    ...
                                    <div class="mt-auto pt-4 border-t border-white/50 flex flex-wrap gap-2">
                                        ...
                                        @if(count($rep->cercanas ?? []) > 0)
                                            <button data-report="{{ json_encode($rep) }}" onclick="openReviewDrawer(this)" class="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 text-sm rounded-xl font-bold shadow-sm transition-colors flex items-center justify-center gap-2 mt-2">
                                                Vincular (Ver Mapa)
                                            </button>
                                        @endif
                                        
                                        <button onclick="validarRapido({{ $rep->id }}, 'rechazar')" class="bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 px-4 py-2 text-sm rounded-xl font-bold shadow-sm transition-colors flex-1">
                                            Rechazar
                                        </button>
                                    </div>
                                </div>
    ...
    <script>
        ...
        function validarRapido(id, action, inundacion_id = null) {
            ...
            Swal.fire({
                title: '¿Estás seguro?',
                text: '¿Estás seguro de ' + action + ' este reporte?',
                icon: 'warning',
    ...
    <script>
        ...
        function openReviewDrawer(btnElement) {
            ...
            document.getElementById('btn-approve').parentElement.style.display = 'none'; // Hide approve/reject in drawer
